...
Page({
  ...
  async login(){
    //1.收集表单数据
    let {email,phone,password} = this.data;
    //2.前端验证手机登陆
    /*
    * 手机号邮箱验证
    *  1.内容为空
    ...
    * */
    ...
    //定义正则表达式
    let phoneReh = /^1(3|4|5|6|7|8|9)\d{9}$/;
    let emailReh = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;

    if (!phoneReh.test(phone) && !emailReh.test(email)){
      wx.showToast({
        title: "格式错误",
        icon: "none"
      })
